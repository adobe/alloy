...
Added a quickstart escape hatch for `@adobe/alloy-node`'s required `edgeCredentials`: passing `edgeCredentials: { clientId: "TEST", clientSecret: "TEST" }` satisfies `configure()`'s check but is discarded rather than used, so requests fall back to the standard unauthenticated v1 endpoint instead of trying (and failing) to authenticate with IMS using fake values. Meant for local exploration and tests, not production.
