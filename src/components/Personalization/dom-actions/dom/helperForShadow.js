...
export const isNotShadowSelector = str => str.indexOf(SHADOW_SEPARATOR) === -1;

export const splitWithShadow = selector => {
  return selector.split(SHADOW_SEPARATOR);
};

export const cleanupPrefix = selector => {
  const result = selector.trim();
  const subselIdx = result.indexOf(">");
  return subselIdx === 0 ? result.substring(1).trim() : result;
};
