...
createFixture({
  title:
    "Test C13412100: Verify that no message is displayed when renderDecision is true & decisionContext doesn't satisfy the ruleset.",
...
test("Test C13412100: Verify that no message is displayed when renderDecision is true & decisionContext doesn't satisfy the ruleset.", async () => {
  ...
  await alloy.applyResponse({
    ...
    decisionContext: {
      "~state.com.adobe.module.lifecycle/lifecyclecontextdata.dayofweek": 4
    },
    responseBody: mockResponse
  });
  const containerElement = await getIframeContainer();
  await t.expect(containerElement.length).eql(0);
});
