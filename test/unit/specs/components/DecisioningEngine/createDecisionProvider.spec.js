...
describe("DecisioningEngine:createDecisionProvider", () => {
  ...
  it("returns a different single payload with items that qualify", async () => {
    const expectedResult = await decisionProvider.evaluate({
  ...
  it("returns two payloads with items that qualify", async () => {
    const expectedResult = await decisionProvider.evaluate({
      color: "blue",
      action: "lipstick",
      "xdm.web.webPageDetails.viewName": "home"
  ...
  it("ignores payloads that aren't json-ruleset type", async () => {
