...
describe("createProcessPropositions", () => {
  ...
  beforeEach(() => {
    ...
    always = item => ({ render: () => render(item.getData()), setRenderAttempted: true, includeInNotification: true });
    noNotification = item => ({ render: () => render(item.getData()), setRenderAttempted: true, includeInNotification: false });
    never = () => ({});
    noRender = () => ({ setRenderAttempted: true, includeInNotification: true });
    redirect = item => ({ render: () => render(item.getData()), setRenderAttempted: true, onlyRenderThis: true});

    schemaProcessors = { always, noNotification, never, noRender, redirect };
    logger = jasmine.createSpyObj("logger", ["info", "error"]);
    processPropositions = createProcessPropositions({ schemaProcessors, logger });
  ...
  it("processes a proposition with an always item", async () => {
    const prop1 = createProposition({ id: "always1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "always", data: "mydata" }] });
    const result = processPropositions([prop1]);
    expect(result).toEqual({ render: jasmine.any(Function), returnedPropositions: [{
      id: "always1",
      scope: "myscope",
      scopeDetails: { a: 1 },
  ...
  it("processes a proposition with a noNotification item", async () => {
    const prop1 = createProposition({ id: "noNotification1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "noNotification", data: "mydata" }] });
    const result = processPropositions([prop1]);
    expect(result).toEqual({ render: jasmine.any(Function), returnedPropositions: [{
      id: "noNotification1",
      scope: "myscope",
  ...
  it("processes a proposition with a never item", async () => {
    const prop1 = createProposition({ id: "never1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "never", data: "mydata" }] });
    const result = processPropositions([prop1]);
    expect(result).toEqual({ render: jasmine.any(Function), returnedPropositions: [{
      id: "never1",
      scope: "myscope",
  ...
  it("processes a proposition with a noRender item", async () => {
    const prop1 = createProposition({ id: "noRender1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "noRender", data: "mydata" }] });
    const result = processPropositions([prop1]);
    expect(result).toEqual({ render: jasmine.any(Function), returnedPropositions: [{
      id: "noRender1",
      scope: "myscope",
  ...
  it("processes a proposition with a redirect item", async () => {
    const prop1 = createProposition({ id: "redirect1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "redirect", data: "mydata" }] });
    const result = processPropositions([prop1]);
    expect(result).toEqual({ render: jasmine.any(Function), returnedPropositions: [{
      id: "redirect1",
      scope: "myscope",
  ...
  it("doesn't render other propositions if one has a redirect", async () => {
    const prop1 = createProposition({ id: "always1", scope: "myscope", scopeDetails: { a: 1 }, items: [ { schema: "always", data: "mydata1" }] });
    const prop2 = createProposition({ id: "redirect2", scope: "myscope", scopeDetails: { a: 2 }, items: [ { schema: "redirect", data: "mydata2" }] });
    const prop3 = createProposition({ id: "always3", scope: "myscope", scopeDetails: { a: 3 }, items: [ { schema: "always", data: "mydata3" }] });
