...
describe("createProcessDomAction", () => {
  ...
  beforeEach(() => {
    ...
    processDomAction = createProcessDomAction({ modules, logger, storeClickMetrics });
  });

  it("returns an empty object if the item has no data, and logs missing type", () => {
    data = undefined;
    expect(processDomAction(item)).toEqual({});
    expect(logger.warn).toHaveBeenCalledWith("Invalid DOM action data: missing type.", undefined);
  });

  it("returns an empty object if the item has no type, and logs missing type", () => {
    data = {};
    expect(processDomAction(item)).toEqual({});
    expect(logger.warn).toHaveBeenCalledWith("Invalid DOM action data: missing type.", {});
  });

  it("returns an empty object if the item has an unknown type, and logs unknown type", () => {
    data = { type: "typeC" };
    expect(processDomAction(item)).toEqual({});
    expect(logger.warn).toHaveBeenCalledWith("Invalid DOM action data: unknown type.", { type: "typeC" });
  });

  it("returns an empty object if the item has no selector for a click type, and logs missing selector", () => {
    data = { type: "click" };
    expect(processDomAction(item)).toEqual({});
    expect(logger.warn).toHaveBeenCalledWith("Invalid DOM action data: missing selector.", { type: "click" });
  });

  it("handles a click type", () => {
    data = { type: "click", selector: ".selector" };
    meta = "mymetavalue";
    expect(processDomAction(item)).toEqual({ setRenderAttempted: true, includeInNotification: false });
    expect(storeClickMetrics).toHaveBeenCalledWith({ selector: ".selector", meta: "mymetavalue" });
  });
