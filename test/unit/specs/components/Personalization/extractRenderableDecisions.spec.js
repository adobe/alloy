...
import extractRenderableDecisions from "../../../../../src/components/Personalization/extractRenderableDecisions";

describe("Personalization::extractDecisions", () => {
  it("extracts dom action decisions and rest of decisions", () => {
    const decisions = PAGE_WIDE_SCOPE_DECISIONS.concat(
      SCOPES_FOO1_FOO2_DECISIONS,
      CART_VIEW_DECISIONS
    );

    const [matchedDecisions, unmatchedDecisions] = extractRenderableDecisions(
      decisions
    );
    ...
    expect(matchedDecisions).toEqual(
      PAGE_WIDE_SCOPE_DECISIONS_WITH_DOM_ACTION_SCHEMA_ITEMS.concat(
        CART_VIEW_DECISIONS
      )
    );
  });

  it("extracts dom action decisions only", () => {
