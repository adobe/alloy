...
import { collectHashedIPAddr } from "../identities/initiateAdvertisingIdentityCall.js";
...
export default async function handleOnBeforeSendEvent({
  ...
  // Only skip identity collection (including collectHashedIPAddr) when disabled or click-through
  if (isAdvertisingDisabled(advertising) || isClickThru) return;
  ...
  try {
    ...
    const [surferIdResult, hashedIPResult, id5IdResult, rampIdResult] =
      await Promise.allSettled([
        ...
        collectHashedIPAddr(cookieManager),
        id5Promise,
        rampIdPromise,
      ]);
    ...
    if (id5IdResult.status === "fulfilled" && id5IdResult.value) {
      availableIdsForIdentity.id5Id = id5IdResult.value;
      if (!isThrottled(ID5_ID, cookieManager)) {
        availableIds.id5Id = id5IdResult.value;
      }
    }
    if (rampIdResult.status === "fulfilled" && rampIdResult.value) {
      //availableIdsForIdentity.rampId = rampIdResult.value;
      if (!isThrottled(RAMP_ID, cookieManager)) {
    ...
    // When all IDs are throttled, availableIds stays empty; skip adding query.advertising
    if (
