...
describe("createLogger", () => {
  let myConsole;
  let logger;
  let now = new Date(2001, 2, 3, 4, 5, 6, 7);
  const prefix = chalk.white("[04:05:06.007]");
  beforeEach(() => {
    myConsole = jasmine.createSpyObj("myConsole", ["log", "info", "warn", "error"]);
  ...
  it("logs warnings yellow", () => {
    logger.warn("mywarn");
    expect(myConsole.warn).toHaveBeenCalledOnceWith(`${prefix} ${chalk.yellow("mywarn")}`);
  });
  it("logs errors red", () => {
    logger.error("myerror");
    expect(myConsole.error).toHaveBeenCalledOnceWith(`${prefix} ${chalk.red("myerror")}`);
  });
